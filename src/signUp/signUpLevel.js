...
export const Level = ({level}) => {
    return (
        <View style={signUpStyles.level_box_box}>
...
const Level_box = ({level,text,num}) => {
    return (
        <View style={[signUpStyles.level_box, {flexDirection:'column'}]} >
            <Level_text/>
...
const Level_text = ({text,color}) => {
    return (
        <Text style={[signUpStyles.level_text, {color:color}]}>{text}</Text>
    )
}